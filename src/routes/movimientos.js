...
async function existeContrapartePrecargada({ cuit, nombre, tipo }) {
  ...
  if (cuitLimpio.length === 11) {
    params.push(cuitLimpio);
    condiciones.push(`regexp_replace(COALESCE(cuit, ''), '[^0-9]', '', 'g') = $${params.length}`);
  }
  if (nombreLimpio) {
    params.push(nombreLimpio);
    condiciones.push(`LOWER(TRIM(razon_social)) = LOWER(TRIM($${params.length}))`);
  }
  ...
  const { rows } = await pool.query(
    `SELECT id
     ...
     WHERE (${condiciones.join(' OR ')})
       AND (tipo_contraparte = $${params.length} OR tipo_contraparte = 'AMBOS')
     LIMIT 1`,
    params
  );
...
async function existeContraparteDesdeContrato(idContrato, tipo) {
  if (!idContrato) return false;
  const { rows } = await pool.query(
    `SELECT c.id
     FROM contratos ct
     JOIN contrapartes c ON ct.id_contraparte = c.id
     WHERE ct.id = $1
       AND (c.tipo_contraparte = $2 OR c.tipo_contraparte = 'AMBOS')
     LIMIT 1`,
    [idContrato, tipo]
  );
  return rows.length > 0;
...
async function existeTransportistaPrecargado({ id_transportista, transportista_nombre, transportista }) {
  if (id_transportista) {
    const { rows } = await pool.query('SELECT id FROM transportistas WHERE id = $1 LIMIT 1', [id_transportista]);
    if (rows.length > 0) return true;
  }

  const nombre = normalizarTexto(transportista_nombre || transportista);
  if (!nombre) return false;
  const { rows } = await pool.query(
    'SELECT id FROM transportistas WHERE LOWER(TRIM(razon_social)) = LOWER(TRIM($1)) LIMIT 1',
    [nombre]
  );
  return rows.length > 0;
}

async function evaluarPendientesConfirmacion(payload) {
  const pendientes = [];

  const productorPrecargado =
    await existeContraparteDesdeContrato(payload.id_contrato_compra, 'PRODUCTOR') ||
    await existeContrapartePrecargada({
      cuit: payload.titular_cpe_cuit,
      nombre: payload.titular_cpe_nombre,
      tipo: 'PRODUCTOR'
    }) ||
    await existeContrapartePrecargada({
      cuit: payload.remitente_comercial_productor_cuit,
      nombre: payload.remitente_comercial_productor_nombre,
      tipo: 'PRODUCTOR'
    });

  if (!productorPrecargado) {
    pendientes.push('PRODUCTOR_NO_PRECARGADO');
  }

  const compradorPrecargado =
    await existeContraparteDesdeContrato(payload.id_contrato_venta, 'COMPRADOR') ||
    await existeContrapartePrecargada({
      cuit: payload.destinatario_cuit,
      nombre: payload.destinatario_nombre,
      tipo: 'COMPRADOR'
    }) ||
    await existeContrapartePrecargada({
      cuit: payload.destino_cuit,
      nombre: payload.destino_nombre,
      tipo: 'COMPRADOR'
    }) ||
    await existeContrapartePrecargada({
      cuit: payload.flete_pagador_cuit,
      nombre: payload.flete_pagador_nombre,
      tipo: 'COMPRADOR'
    });

  if (!compradorPrecargado) {
    pendientes.push('COMPRADOR_NO_PRECARGADO');
  }

  const transportistaPrecargado = await existeTransportistaPrecargado(payload);
  if (!transportistaPrecargado) {
    pendientes.push('TRANSPORTISTA_NO_PRECARGADO');
  }
  ...
  try {
    ...
    query += `
      ORDER BY CASE 
        ...
        WHEN m.estado = 'BORRADOR' THEN 2
        WHEN m.estado = 'EN_TRANSITO' THEN 3
        WHEN m.estado = 'DESCARGADO' THEN 4
        WHEN m.estado = 'LIQUIDADO' THEN 5
        ELSE 6
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!finalTransportista && id_transportista) {
      const { rows: tRows } = await pool.query('SELECT razon_social FROM transportistas WHERE id = $1', [id_transportista]);
      if (tRows[0]) finalTransportista = tRows[0].razon_social;
    }
    if (!finalChofer && id_chofer) {
      const { rows: cRows } = await pool.query('SELECT nombre_completo FROM choferes WHERE id = $1', [id_chofer]);
      if (cRows[0]) finalChofer = cRows[0].nombre_completo;
    }
    ...
    const pendientesConfirmacion = await evaluarPendientesConfirmacion({
      id_contrato_compra,
      id_contrato_venta,
      titular_cpe_cuit,
      titular_cpe_nombre,
      remitente_comercial_productor_cuit,
      remitente_comercial_productor_nombre,
      destinatario_cuit,
      destinatario_nombre,
      destino_cuit,
      destino_nombre,
      flete_pagador_cuit,
      flete_pagador_nombre,
      id_transportista,
...
// PUT confirmar movimiento pendiente
router.put('/:id/confirmar', async (req, res) => {
  try {
    const { rows: movs } = await pool.query('SELECT * FROM movimientos WHERE id = $1', [req.params.id]);
    if (!movs[0]) return res.status(404).json({ error: 'No encontrado' });

    const m = movs[0];
    const pendientesConfirmacion = await evaluarPendientesConfirmacion({
      id_contrato_compra: m.id_contrato_compra,
      id_contrato_venta: m.id_contrato_venta,
      ...
      id_transportista: m.id_transportista,
      transportista_nombre: m.transportista_nombre
    });
...
router.put('/:id', async (req, res) => {
  try {
    ...
    let motivoPendiente = currentMov[0].motivo_pendiente;
    if (currentMov[0].estado === 'PENDIENTE') {
      const pendientesConfirmacion = await evaluarPendientesConfirmacion({
        id_contrato_compra,
        id_contrato_venta,
        titular_cpe_cuit,
